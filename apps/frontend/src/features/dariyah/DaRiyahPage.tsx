...
const STATIC_MODELS: ModelInfo[] = [
  { id: 'claude-opus-4-6',   label: 'Claude Opus 4.6',   provider: 'anthropic', requires_key: 'ANTHROPIC_API_KEY', key_set: false },
  { id: 'claude-sonnet-4-6', label: 'Claude Sonnet 4.6', provider: 'anthropic', requires_key: 'ANTHROPIC_API_KEY', key_set: false },
  { id: 'claude-haiku-4-5',  label: 'Claude Haiku 4.5',  provider: 'anthropic', requires_key: 'ANTHROPIC_API_KEY', key_set: false },
  { id: 'gemini-2.0-flash',  label: 'Gemini 2.0 Flash',  provider: 'google',    requires_key: 'GOOGLE_AI_API_KEY', key_set: false },
]

async function fetchModels(): Promise<ModelInfo[]> {
  try {
    return await callEdgeFunction<ModelInfo[]>('dariyah-models', {})
  } catch {
    return STATIC_MODELS
  }
}

// Da'Riyah standalone intelligence — runs entirely in browser, zero API calls
function dariyahLocal(message: string): string {
  const m = message.toLowerCase()
  if (m.includes('royalt') || m.includes('stream') || m.includes('earn') || m.includes('payout')) {
    return `**DMF Royalty Math — 2026 Real Rates:**\n\n| Platform | Rate/Stream | 50k streams | 200k streams |\n|---|---|---|---|\n| Spotify | $0.004 | $200 | $800 |\n| Apple Music | $0.010 | $500 | $2,000 |\n| YouTube | $0.0008 | $40 | $160 |\n| Tidal | $0.013 | $650 | $2,600 |\n\n**Path to $5k/month:** ~1.25M Spotify streams OR ~500k Apple streams.\n\nSet ANTHROPIC_API_KEY in your Supabase Edge Function to unlock full Claude analysis.\n\n*West Side built, worldwide hustle. — Da'Riyah*`
  }
  if (m.includes('roster') || m.includes('artist') || m.includes('freezzo') || m.includes('obmb') || m.includes('savage') || m.includes('ellumf') || m.includes('big homie')) {
    return `**DMF Roster — March 2026:**\n\n**Big Homie Cash** — Anchor. Most catalog. Push best-of playlist to Columbus curators.\n**Freezzo** — Workhorse. Submit "Calling my cellular" + "IDGAF" to hip-hop playlists.\n**OBMB DELO** — Most underserved. "Standing on my own 10" EP for editorial.\n**Go Savage** — "Pistol on da dresser" hook is TikTok gold. 20 clips in 30 days.\n**Ellumf** — "October 3" Indian fusion hits niche DSP tags with less competition.\n\nClaim all 5 Spotify for Artists profiles — free, immediate.\n\n*West Side built, worldwide hustle. — Da'Riyah*`
  }
  if (m.includes('plan') || m.includes('90') || m.includes('growth') || m.includes('roadmap') || m.includes('strategy')) {
    return `**DMF 90-Day Roadmap:**\n\n**Days 1-30:**\n- Claim Spotify for Artists (all 5 roster members)\n- Register Big Homie Cash + Freezzo with ASCAP/BMI (free)\n- Submit top 3 tracks per artist to SubmitHub ($45-75 total)\n- Post 20 TikTok clips across the roster\n\n**Days 31-60:**\n- Release 1 new single (Freezzo — highest momentum)\n- Run $100 TikTok Spark Ad on best organic clip\n- Build DMF roster playlist on Spotify\n\n**Days 61-90:**\n- Launch 1 merch item via Printful/Shopify ($0 upfront)\n- Plan collab track between 2 roster members\n- Apply for Ohio Arts Council grant\n\n*West Side built, worldwide hustle. — Da'Riyah*`
  }
  return `Locked in, Big Homie. Ask me about:\n- **Royalties** — exact 2026 DSP rates + revenue math\n- **Roster** — artist-by-artist strategy\n- **90-Day Plan** — full DMF growth roadmap\n- **Distribution** — DistroKid vs TuneCore vs UnitedMasters\n- **TikTok/Campaigns** — 30-day sprint plan\n\nTo unlock full Claude AI: add ANTHROPIC_API_KEY to your Supabase Edge Function secrets.\n\n*West Side built, worldwide hustle. — Da'Riyah*`
}

async function chat(req: ChatRequest) {
  try {
    return await callEdgeFunction<{ response: string; model: string; provider: string }>('dariyah-chat', req)
  } catch {
    return { response: dariyahLocal(req.message), model: 'da-riyah-local', provider: 'local' }
  }
}
