...
const SEED_ARTISTS: Artist[] = [
  { id: '1', name: 'Big Homie Cash', real_name: 'Deangelo Jackson', role: 'Founder / Lead Artist', genre: 'Hip-Hop / Rap / Street Rap', location: 'Columbus, OH (West Side)', vibe: 'Hustle anthems, real talk, the come-up documented live', bio: 'Founder of DMF Records Fly Hoolie Ent. Columbus West Side. Building a label from the ground up — every release, every move, on record.', spotify_artist_id: '40z5aBKSs2Wtdori0baO1l', spotify_url: 'https://open.spotify.com/artist/40z5aBKSs2Wtdori0baO1l', key_collabs: ['Freezzo', 'Ellumf', 'B Hus from da bus'] },
  { id: '2', name: 'Freezzo', real_name: undefined, role: 'Core Artist', genre: 'Hip-Hop / Rap / Trap', location: 'Columbus, OH', vibe: 'Hard trap, raw bars, collab machine', bio: "DMF's most prolific collab artist. Consistent drops, real street energy, catalog growing fast.", spotify_artist_id: '4ksrusI7XnIdyuN6a3LtMj', spotify_url: 'https://open.spotify.com/artist/4ksrusI7XnIdyuN6a3LtMj', key_collabs: ['Big Homie Cash', 'Go Savage', 'Chef Lo'] },
  { id: '3', name: 'OBMB DELO', real_name: undefined, role: 'Core Artist', genre: 'Hip-Hop / Rap (Experimental)', location: 'Columbus, OH', vibe: 'Alternative depth, introspective, pushing sonic limits', bio: 'The experimental side of DMF. "Standing on my own 10" showed editorial potential. Most underutilized artist in the roster — that changes now.', spotify_artist_id: '6yjdymBNWSyr39uuuweOfT', spotify_url: 'https://open.spotify.com/artist/6yjdymBNWSyr39uuuweOfT', key_collabs: ['Big Homie Cash', 'Yogi Bear'] },
  { id: '4', name: 'Go Savage', real_name: undefined, role: 'Core Artist', genre: 'Hip-Hop / Rap / Street Rap', location: 'Columbus, OH', vibe: 'Gritty, aggressive, no filter — for the streets', bio: 'Raw street energy that converts on TikTok. "Pistol on da dresser" hook has viral potential. No hook projects built for the culture.', spotify_artist_id: '5qGClg4MZsh2r5ZD88rtEZ', spotify_url: 'https://open.spotify.com/artist/5qGClg4MZsh2r5ZD88rtEZ', key_collabs: ['Ellumf', 'Freezzo'] },
  { id: '5', name: 'Ellumf', real_name: undefined, role: 'Core Artist', genre: 'Hip-Hop / Rap / Alternative Rap', location: 'Columbus, OH', vibe: 'Versatile, genre-bending, built for crossover', bio: '"October 3" Indian fusion is a DSP niche play — hits playlists with less competition. Shots Fire shows range. Underrated in the catalog.', spotify_artist_id: null, spotify_url: null, key_collabs: ['Go Savage', 'Big Homie Cash'] },
]

const SEED_METRICS: SpotifyMetric[] = [
  { artist_id: '1', name: 'Big Homie Cash', spotify_artist_id: '40z5aBKSs2Wtdori0baO1l', followers: 42,  popularity: 18 },
  { artist_id: '2', name: 'Freezzo',        spotify_artist_id: '4ksrusI7XnIdyuN6a3LtMj', followers: 31,  popularity: 15 },
  { artist_id: '3', name: 'OBMB DELO',      spotify_artist_id: '6yjdymBNWSyr39uuuweOfT', followers: 8,   popularity: 6  },
  { artist_id: '4', name: 'Go Savage',      spotify_artist_id: '5qGClg4MZsh2r5ZD88rtEZ', followers: 19,  popularity: 9  },
  { artist_id: '5', name: 'Ellumf',         spotify_artist_id: null,                     followers: null, popularity: null },
]

async function fetchArtists(): Promise<Artist[]> {
  const { data, error } = await supabase.from('artists').select('*').order('name')
  if (error || !data?.length) return SEED_ARTISTS
  return data
}

async function fetchRosterMetrics(): Promise<SpotifyMetric[]> {
  const { data, error } = await supabase.from('spotify_metrics').select('*')
  if (error || !data?.length) return SEED_METRICS
  return data
}
...
export default function ArtistsPage() {
  ...
  if (error) return <div className="p-8 text-red-600 text-sm">Failed to load artists.</div>
